...
#define BUFFER_USAGE GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_HW_CAMERA_WRITE | GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_SW_READ_OFTEN

// Arbitrary limit on number of graphics buffers allowed to be allocated
// Safeguards against unreasonable resource consumption and provides a testable limit
const unsigned MAX_BUFFERS_IN_FLIGHT = 20;


EvsCamera::EvsCamera(const char *id, uint32_t dWidth, uint32_t dHeight) :
        mWidth(dWidth),
        mHeight(dHeight),
        mFramesAllowed(0),
        mFramesInUse(0),
...
{
    if (mFd != -1) {
        ALOGE("Can't open device '%s'", deviceName);
        return false;
    }
...
void EvsCamera::generateFrames() {
    ...
    while (true) {
        ...
        if (timeForFrame) {
            ...
            buff.height     = mHeight;
            buff.stride     = mStride;
            buff.pixelSize  = BYTES_PER_PIXEL;
            buff.format     = HAL_PIXEL_FORMAT_RGBA_8888;
            buff.usage      = mUsage;
            buff.bufferId   = idx;
