...
#define CONNECTOR_ID "/sys/class/drm/card0-VGA-1/connector_id"
#define RESOLUTION_PROPERTY "ro.boot.display.res.VGA"
#else
#define CONNECTOR_ID "/sys/class/drm/card0-HDMI-A-2/connector_id"
#define RESOLUTION_PROPERTY "ro.boot.display.res.HDMI1"
#endif

#define NUM_BUFFERS (3)
#define BUFFER_FORMAT HAL_PIXEL_FORMAT_BGRX_8888
#define BEFFER_USAGE (GRALLOC1_PRODUCER_USAGE_GPU_RENDER_TARGET | GRALLOC1_PRODUCER_USAGE_CPU_WRITE_OFTEN)
...
Return<EvsResult> EvsDisplay::setDisplayState(DisplayState state) {
    ...
    else if (state == DisplayState::NOT_VISIBLE) {
...
Return<void> EvsDisplay::getTargetBuffer(getTargetBuffer_cb _hidl_cb)  {
    ...
    if (mBuffers[mCurrentBuffer].state == NOT_ALLOCATED) {
        ...
        mBuffers[mCurrentBuffer].buffer.pixelSize = 4;
        mBuffers[mCurrentBuffer].buffer.usage = BEFFER_USAGE;
