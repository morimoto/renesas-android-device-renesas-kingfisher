...
struct StreamOut : public IStreamOut {
    ...
    int setFmParametersInternal(const hidl_vec<ParameterValue>& parameters);
    int setVolumeParametersInternal(const hidl_vec<ParameterValue>& parameters);

    enum {
        CUSTOM_FIRST = 1000,

        FM_DISCONNECTED = CUSTOM_FIRST,
        FM_CONNECTED,
        FM_VOLUME,
        FM_BUFFER_READY,

        HFP_DISCONNECTED = CUSTOM_FIRST + 100,
        HFP_CONNECTED,
        HFP_BUFFER_READY,
        MIC_BUFFER_READY,
    };

    struct SHfpBufferSize {
        size_t mSize;
    };

    struct SHfpBuffer {
        SHfpBufferSize mSize;
        char* mData;
    };
  ...
  private:
    ...
    std::unique_ptr<DataMQ> mDataHfpMQ;
    std::unique_ptr<StatusMQ> mStatusMQ;
    EventFlag* mEfGroup;

    std::atomic<bool> mStopWriteThread;
    std::atomic<bool> mStopFmReadThread;
    std::atomic<bool> mStopHfpReadThread;
    std::atomic<bool> mStopMicReadThread;
    std::atomic<bool> mStopHfpWriteThread;
    sp<Thread> mWriteThread;
    sp<CommonStreamOutThread> mFmReadThread;
    sp<CommonStreamOutThread> mHfpReadThread;
    sp<CommonStreamOutThread> mMicReadThread;
    sp<CommonStreamOutThread> mHfpWriteThread;

    std::unique_ptr<CommandMQ> mFmCommandMQ;
    std::unique_ptr<DataMQ> mFmDataMQ;
    std::unique_ptr<StatusMQ> mFmStatusMQ;
    EventFlag* mFmEfGroup;

    std::unique_ptr<CommandMQ> mHfpReadCommandMQ;
    std::unique_ptr<DataMQ> mHfpReadDataMQ;
    std::unique_ptr<StatusMQ> mHfpReadStatusMQ;
    EventFlag* mHfpReadEfGroup;

    std::unique_ptr<CommandMQ> mMicCommandMQ;
    std::unique_ptr<DataMQ> mMicDataMQ;
    std::unique_ptr<StatusMQ> mMicStatusMQ;
    EventFlag* mMicEfGroup;

    std::unique_ptr<CommandMQ> mHfpWriteCommandMQ;
    std::unique_ptr<DataMQ> mHfpWriteDataMQ;
    std::unique_ptr<StatusMQ> mHfpWriteStatusMQ;
    EventFlag* mHfpWriteEfGroup;
    ...
    uint32_t mHfpSampleRate;

    float mOutVolume;
    float mDefaultOutVolume;
