...
Device::Device(audio_hw_device_t* device)
        : mDevice(device), mCurrentHalStream(nullptr), mCurrentStreamOut(nullptr) {
}
...
void Device::closeOutputStream(audio_stream_out_t* stream) {
    if (stream == mCurrentHalStream) {
        mCurrentHalStream = nullptr;
        mCurrentStreamOut = nullptr;
    }
...
Return<void> Device::openOutputStream(int32_t ioHandle,
    ...
    if (status == OK) {
        ...
        mCurrentHalStream = halStream;
        mCurrentStreamOut = streamOut;
    }
...
Return<void> Device::openInputStream(int32_t ioHandle,
                                     const DeviceAddress& device,
                                     const AudioConfig& config,
                                     AudioInputFlagBitfield flags, AudioSource source,
...
Return<Result> Device::setParameters(
    ...
    Result res = setParametersImpl(parameters);
	if ((mCurrentStreamOut.get() != nullptr) && (mCurrentHalStream != nullptr)) {
	    mCurrentStreamOut->setParameters(parameters);
	}
	return res;
}
#elif defined(AUDIO_HAL_VERSION_4_0)
Return<Result> Device::setParameters(const hidl_vec<ParameterValue>& context,
                                     const hidl_vec<ParameterValue>& parameters) {
    Result res = setParametersImpl(context, parameters);
    if ((mCurrentStreamOut.get() != nullptr) && (mCurrentHalStream != nullptr)) {
        mCurrentStreamOut->setParameters(context, parameters);
    }
    return res;
}
#endif
