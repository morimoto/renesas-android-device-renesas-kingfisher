...
VehicleHalImpl::~VehicleHalImpl(void)
{
    ...
    if (mCanThread.joinable()) mCanThread.join();
...
{
    if (mCanThreadExit || mSocket == -1) return;
...
void VehicleHalImpl::CanTxBytes(void * bytesPtr, size_t bytesCount)
{
    ...
    memcpy(&frame.data, bytesPtr, frame.can_dlc);
